 ...
 */
public final class BungeeFileBackend extends AbstractPermissionBackend {
    ...
    public int getSchemaVersion() {
        synchronized (lock) {
            Object v = yamlRoot.get(BungeeYamlMaps.SCHEMA_VERSION);
    ...
    protected void setSchemaVersion(int version) {
        synchronized (lock) {
            yamlRoot.put(BungeeYamlMaps.SCHEMA_VERSION, version);
            persistIfNeeded();
        }
    }
    ...
    private boolean ensureDefaultGroupsAndSchemaPresent() {
        boolean mutated = false;
        Map<String, Object> groups = BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.GROUPS);
        if (groups == null || groups.isEmpty()) {
            yamlRoot.put(BungeeYamlMaps.GROUPS, newDefaultGroupsTree());
            mutated = true;
        }
        if (!yamlRoot.containsKey(BungeeYamlMaps.SCHEMA_VERSION)) {
            yamlRoot.put(BungeeYamlMaps.SCHEMA_VERSION, Math.max(getLatestSchemaVersion(), 1));
            mutated = true;
        }
        return mutated;
    }
    ...
    private static Map<String, Object> newDefaultGroupsTree() {
        ...
        defGrp.put(BungeeYamlMaps.OPTIONS, defOpts);
        List<String> defaultPermissions = new ArrayList<>();
        defaultPermissions.add("modifyworld.*");
        defGrp.put(BungeeYamlMaps.PERMISSIONS, defaultPermissions);
    ...
    private void hydrateWorldInheritance() {
        ...
        Map<String, Object> legacy =
                BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.WORLD_INHERITANCE_LEGACY);
        if (legacy != null) {
            mergeLegacyWorldInheritance(legacy);
        }

        Map<String, Object> topWorlds = BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.WORLDS);
        if (topWorlds != null) {
    ...
    private void mergeLegacyWorldInheritance(Map<String, Object> legacy) {
        for (Map.Entry<String, Object> e : legacy.entrySet()) {
            putWorldInheritanceHydrate(e.getKey(), BungeeYamlMaps.coerceToStringList(e.getValue()));
        }
    }
    ...
    private void mergeTopWorldsInheritance(Map<String, Object> topWorlds) {
        for (Map.Entry<String, Object> e : topWorlds.entrySet()) {
            ...
            putWorldInheritanceHydrate(e.getKey(), BungeeYamlMaps.coerceToStringList(inh));
        }
    }
     ...
     * Spigot FileBackend). Nested {@code worlds/&lt;name&gt;/inheritance} is updated in {@link #setWorldInheritance}.
     */
    private void persistLegacyWorldInheritanceBlock() {
        if (worldInheritanceCache.isEmpty()) {
            yamlRoot.remove(BungeeYamlMaps.WORLD_INHERITANCE_LEGACY);
            return;
        }
        Map<String, Object> wi =
                BungeeYamlMaps.requireSection(yamlRoot, BungeeYamlMaps.WORLD_INHERITANCE_LEGACY);
        wi.clear();
    ...
    public PermissionsUserData getUserData(String userName) {
        synchronized (lock) {
            Map<String, Object> users = BungeeYamlMaps.requireSection(yamlRoot, BungeeYamlMaps.USERS);
            var data = BungeeYamlEntityData.forUser(this, users, userName);
            data.load();
            return data;
        }
    }
    ...
    public PermissionsGroupData getGroupData(String groupName) {
        synchronized (lock) {
            Map<String, Object> groups =
                    BungeeYamlMaps.requireSection(yamlRoot, BungeeYamlMaps.GROUPS);
            var data = BungeeYamlEntityData.forGroup(this, groups, groupName);
            data.load();
            return data;
        }
    }
    ...
    public boolean hasUser(String userName) {
        synchronized (lock) {
            Map<String, Object> users =
                    BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.USERS);
    ...
    public boolean hasGroup(String group) {
        synchronized (lock) {
            Map<String, Object> groups =
                    BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.GROUPS);
            if (groups == null) {
                return false;
            }
    ...
    private static String nestedOptionLeaf(Map<String, Object> entityRoot, String world,
        ...
        Map<String, Object> bucket =
                world == null || world.isEmpty() ? entityRoot
                        : BungeeYamlMaps.worldBucket(entityRoot, world);
        Map<String, Object> opts = BungeeYamlMaps.getSection(bucket, BungeeYamlMaps.OPTIONS);
    ...
    private List<String> readWorldInheritanceFromYamlUnsynced(String world) {
        ...
        return List.copyOf(BungeeYamlMaps.coerceToStringList(wSec.get("inheritance")));
    }
    ...
    public void setWorldInheritance(String world, List<String> rawParentWorlds) {
        ...
        synchronized (lock) {
            ...
            if (parentWorlds.isEmpty()) {
                ...
                Map<String, Object> tw = BungeeYamlMaps.getSection(yamlRoot, BungeeYamlMaps.WORLDS);
                if (tw != null) {
                    tw.remove(world);
                    if (tw.isEmpty()) {
                        yamlRoot.remove(BungeeYamlMaps.WORLDS);
                    }
                }
                ...
                Map<String, Object> tw =
                        BungeeYamlMaps.requireSection(yamlRoot, BungeeYamlMaps.WORLDS);
                Map<String, Object> wSec = BungeeYamlMaps.requireSection(tw, world);
    ...
    private void persistWorldMirrorsForDump(LinkedHashMap<String, Object> rootCopy) {
        Map<String, Object> wi =
                BungeeYamlMaps.requireSection(rootCopy, BungeeYamlMaps.WORLD_INHERITANCE_LEGACY);
        wi.clear();
        for (Map.Entry<String, List<String>> e : worldInheritanceCache.entrySet()) {
            if (!e.getValue().isEmpty()) {
                wi.put(e.getKey(), new ArrayList<>(e.getValue()));
        ...
        if (wi.isEmpty()) {
            rootCopy.remove(BungeeYamlMaps.WORLD_INHERITANCE_LEGACY);
        }
    }
