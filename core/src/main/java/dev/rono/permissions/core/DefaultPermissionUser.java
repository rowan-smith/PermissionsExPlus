 ...
 * @author code
 */
public class DefaultPermissionUser extends AbstractPermissionEntity implements PermissionUser {
	...
	public DefaultPermissionUser(String playerName, PermissionsUserData data, PermissionManager manager) {
	...
	protected void swapGroups(PermissionGroup src, PermissionGroup dst) {
		...
		// Rank transitions must update direct assignments only; inherited parents are derived.
		List<PermissionGroup> groups = new ArrayList<>(this.getOwnParents());
